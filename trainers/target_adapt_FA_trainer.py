...
from utils import IterationCounter, Visualizer, mean_dice, mean_dice_new, mean_asd
...
class FA_Trainer():
    ...
    def initialize(self):
        ...
        ## losses
        self.criterion_proto = My_ProtoLoss(self.opt['nav_t'],self.opt['beta'],self.opt['num_classes'],self.opt['gpu_id'],self.opt['s_par'])
    ...
    def get_optimizers(self):
        ...
        scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=1, gamma=0.8)#0.8
    ...
    def train_one_step(self, data):
        ...
        #target_f = F.interpolate(target_f,size=imgs.shape[-2:],mode='bilinear')###
                    ...
                    if self.iter_counter.needs_evaluation_steps():
                        ...
                        for it, (val_imgs, val_segs, val_names) in enumerate(val_iterator):
                            ...
                            if 'eval_2d' not in self.opt:
                                ...
                                for i,name in enumerate(val_names):
                                    sample_list.append((predict[i].detach().cpu().argmax(dim=0),val_segs[i].detach().cpu()))
                        ...
                        if 'Curvas' in self.opt['experiment_name']:
                            val_metrics['dice'] = mean_dice_new(pred_results_list,gt_segs_list,3,['kidney','Liver'])   
                            val_metrics['asd'] = mean_asd(pred_results_list,gt_segs_list,3,['kidney','Liver'])
                        else:
